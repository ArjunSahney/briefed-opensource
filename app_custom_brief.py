# Solely manages 18 custom briefs per person: 
...
def generate_morning_briefing(name, briefing_dictionary, use_gpt=False):
    ...
    custom_dictionary = {}
    general_dictionary = {}
    # TOP HEADLINES -- APP GENERAL
    if briefing_dictionary["Top Headlines"] is not None:
        num_headline_briefs_actual = len(briefing_dictionary["Top Headlines"])
        # Getting all top headlines in main brief ## MOVE TO GENERAL BRIEF
        general_dictionary = briefing_dictionary["Top Headlines"]
        podcast_dictionary = briefing_dictionary["Top Headlines"][:num_headline_briefs_actual//2]
        

    # CUSTOM HEADLINES
    custom_dictionary["Custom Headlines"] = {}
    podcast_dictionary["Custom Headlines"] = {}
    for topic, topic_briefs in briefing_dictionary["Custom Headlines"].items():
        if topic_briefs is not None:
            ...
            custom_dictionary["Custom Headlines"][topic] = topic_briefs
            podcast_dictionary["Custom Headlines"][topic] = topic_briefs[:num_topic_briefs//2]
    ...
    if "Trending Headlines" in briefing_dictionary:
        if briefing_dictionary["Trending Headlines"] is not None:
            ...
            general_dictionary["Trending Headlines"] = briefing_dictionary["Trending Headlines"][num_trending_briefs_actual]
            podcast_dictionary["Trending Headlines"] = briefing_dictionary["Trending Headlines"][:num_trending_briefs_actual//2]
    ...
    filename = "app_brief_files/podcat_dict_" + name + "_" + curr_date + ".txt"
    with open(filename, 'w') as file:
        file.write(json.dumps(podcast_dictionary, indent=4))
...
Please create the morning briefing following these guidelines and incorporating the provided news updates.
        {json.dumps(podcast_dictionary, indent=4)}
        """
        ...
        morning_briefing = format_headlines(podcast_dictionary)
    ...
    filename = "brief_files/morning_briefing" + name + "_" + curr_date + ".txt"
    with open(filename, 'w') as file:
    ...
    mp3_url = upload_mp3_file('briefed_mvp3_mp3', audio_filename)
...
def in_morning_brief(name, company, industry, topic): 
    ...
    headline_filename = "app_brief_files/headlines_" + curr_date + ".txt"
    ...
    # Get trending headlines from trending.txt ##CHECKED
    trending_filename = "app_brief_files/trending_" + curr_date + ".txt"
